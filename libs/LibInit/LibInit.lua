...
--
lib.pool = lib.pool or setmetatable({}, {
    __mode = "k",
...
function lib:NewAddon(target, ...)
    ...
    for i = start, select('#', ...) do
        ...
        if type(mix) == "boolean" and mix then
            for n, k in LibStub:IterateLibraries() do
                if (n:match("Ace%w*-3%.0") and k.Embed) then
                    appo[n] = true
                end
            end
    ...
    if customOptions then
        for k, v in pairs(customOptions) do
            ...
            if key == "enhanceprofile" then
                key = "enhancedprofile"
            end
            if key == "profile" or key == "noswitch" or key == "nogui" or key == "nohelp" or key == "enhancedprofile" then
                options[key] = v
            else
...
function lib:NewSubModule(name, ...)
    ...
    obj.OnInitialize = function(self, ...)
...
function lib:GetMissionInfo(id)
    ...
    return t.location, t.xp, t.environment, t.environmentDesc, t.environmentTexture, t.locTextureKit, t.isExhausting, t.enemies
end
...
function lib:GetCurrencyInfo(id)
    ...
    return t.name, t.quantity, t.iconFileID, t.quantityEarnedThisWeek, t.maxWeeklyQuantiti, t.maxQuantity, t.discovered, t.quality
end
...
function lib:Notice(...)
    if self.db and not self.db.silent then
...
function lib:GetChatFrame(chat)
    if (chat) then
        if (lib.chats[chat]) then
            return lib.chats[chat]
        end
...
local function SetupProfileSwitcher(tbl, addon)
    local profiles = tbl.handler:ListProfiles({
        args = "both"
    })
...
function lib:OnInitialize(...)
    ...
    if self.db then
        ...
        if (not self.db.global.silent) then
            self:Print(format("Version %s %s loaded (%s)", self:Colorize(options.version, 'green'), self:Colorize(format("(Revision: %s)", options.revision), "silver"), "Disable message with /" .. strlower(options.ID) .. " silent"))
    ...
    if (type(self.LoadHelp) == "function") then
        self:LoadHelp()
    end
...
function lib:HF_CmdA(command, description, tooltip)
    self:HF_Push(nil, C('/' .. command, 'orange') .. ' : ' .. (description or '') .. '\n' .. C(tooltip or '', 'yellow'))
end
...
local function _GetMethod(target, prefix, func)
    if (func == 'Start' or func == 'Stop') then
        return
    end
...
function lib:Long(msg)
    C:OnScreen('Yellow', msg, 20)
end
function lib:Onscreen_Orange(msg)
    C:OnScreen('Orange', msg, 2)
end
function lib:Onscreen_Purple(msg)
    C:OnScreen('Purple', msg, 8)
end
function lib:Onscreen_Yellow(msg)
    C:OnScreen('Yellow', msg, 1)
end
function lib:Onscreen_Azure(msg)
    C:OnScreen('Azure', msg, 1)
end
function lib:Onscreen_Red(msg)
    C:OnScreen('Red', msg, 1)
end
function lib:Onscreen_Green(msg)
    C:OnScreen('Green', msg, 1)
end
function lib:OnScreen(color, ...)
    C:OnScreen(color, strjoin(' ', tostringall(...)))
end
...
function lib:Empty(obj)
    if not obj then
        return true
    end -- Simplest case, obj evaluates to false in boolean context
    local t = type(obj)
    if t == "number" then
        return obj == 0
    elseif t == "bool" then
...
local function StopSpellCasting(this)
    ...
    AC:SetScript("PostClick", function()
        b2:Click()
    end)
    AC:Show()
end
...
function lib:coroutineExecute(interval, action, combatSafe, ...)
    ...
    if type(c.co) == "thread" and coroutine.status(c.co) == "suspended" then
        return signature
    end
...
function lib:SetCustomEnvironment(new_env)
    ...
    if getmetatable(new_env) == meta then
        return
    end
    if not getmetatable(new_env) then
        if not new_env.print then
            new_env.print = dprint
        end
