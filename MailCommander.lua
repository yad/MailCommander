...
function addon:InitData()
            ...
            for name, key in pairs(d:GetCharacters(realm)) do
                ...
                if name ~= thisToon then -- Do not overwrite current data with (possibly) stale data
                    if d:IsEnabled("DataStore_Characters") then
                        ...
                        if not db.toons[name] then
                            db.toons[name] = {}
                        end
                        if faction then
                            db.toons[name].faction = faction
                        end
                        if localizedClass then
                            db.toons[name].localizedClass = localizedClass
                        end
                        if class then
                            db.toons[name].class = class
                        end
                        if level then
                            db.toons[name].level = level
                        end
                        if p1 then
                            db.toons[name].p1 = p1
                        end
  ...
  --@end-debug@]===]
    ...
    mcf.HookOpenAllBags = function(self, ...)
        print("MCFHOOK", self, ...)
    end
    self:SetAdditional()
    self.xdb = db
...
function addon:ApplyALLFACTIONS(value)
    self:loadToonList()
    if MailCommanderFrame:IsVisible() then
        self:UpdateMailCommanderFrame()
    end
end
...
function addon:MigrateDatabase()
    ...
    if self.db.global.failedUpdate then
        self:Popup(C("Mailcommander", "Orange") .. "\n" .. L["Was not able to migrate your database."] .. "\n" .. L["Should I erase your old data?"], function(this)
            C_Timer.After(0.5, addon.Reset)
        end, function()
        end)
        return
    end
...
function addon:OnDatabaseShutdown()
    ...
    checkBags()
    if type(db.updateStock) ~= "table" then
        db.updateStock = {}
    end
...
function addon:OnLoad(frame)
    ...
    UIDropDownMenu_Initialize(frame.Filter, function(...)
...
function addon:GetFilter()
    if currentTab == INEED then
        ...
        if not self:GetBoolean("ALLSEND") and not sendable[currentReceiver] then
            ...
            if currentReceiver == thisToon then
                currentReceiver = nil
            end
        end
    ...
    for _, name in ipairs(t) do
        ...
        if realm ~= data.realm then
            ...
            if type(realm) == "nil" then
                self:applyDEBUG("Realm is nil for", data.name)
            end
            info.isTitle = true
            info.notCheckable = true
...
function addon:RenderButtonList(store, page)
    mcf.store = store
    if currentRequester == thisToon then
        mcf.Delete:Disable()
    else
        mcf.Delete:Enable()
    end
    ...
    if store then
        ...
        for itemId, forbidden in pairs(store) do
            if currentTab == INEED or currentTab == ICATEGORIES or (currentTab == IFILTER and (toonTable[forbidden].level or maxLevel) >= self:GetNumber("MINLEVEL")) or (currentTab == ISEND and (self:GetBoolean('ALLSEND') or Count:Sendable(itemId) > 0)) then
                if i > first then
                    if i > last then
                        nextpage = true
                        break
...
function addon:DeleteStore()
    if currentRequester and currentRequester ~= NONAME then
        self:Popup(C("Mailcommander", "Orange") .. "\n" .. format(L["Do you want to delete %1$s\nfrom DataStore, too?"] .. "\n" .. L["If you dont remove %1$s also from DataStore, it will be back"], currentRequester), DeleteStore, function()
            currentRequester = NONAME
            addon:UpdateMailCommanderFrame()
...
function addon:OnDeleteClick(this, button)
    ...
    if info then
        self:Popup(C("Mailcommander", "Orange") .. "\n" .. format(L["Do you want to delete\n%s?"], info.text), DeleteToon, function()
        end, currentRequester)
    end
end
...
function addon:OnItemClicked(itemButton, button)
    ...
    elseif section == "presets" then
        if currentTab == INEED then
            ...
            if db.toons[currentRequester].requests[key] then
                return
            end
...
function addon:ClickedOnToon(itemButton, button)
    ...
    elseif button == "RightButton" then
        return self:Popup(C("Mailcommander", "Orange") .. "\n" .. format(L["Do you want to delete\n%s?"], toonTable[name].text), DeleteToon, function()
        end, name)
    end
...
function addon:ShowSplitter(key, toon, itemButton, itemId, r, g, b)
    ...
    if StackSplitFrame.split > 0 then
    ...
    elseif currentTab == INEED then
        if button == "LeftButton" then
            if (itemId and toon) then
                self:Debug(alt)
                if type(db.toons[toon].requests[itemId]) ~= "table" then
                    db.toons[toon].requests[itemId] = {}
                end
                if alt then
        ...
        if itemlink then
            ...
            if currentTab == ICATEGORIES then
                ...
                if currentTab == INEED then
                    ...
                    if disabled then
                        GameTooltip:AddLine(format(L["Disabled items are not sent with \"%s\" button"], L["Send All"]), C:Orange())
                    end
...
function addon:UpdateMailCommanderFrame()
    if mcf.selectedTab == INEED then
        ...
        UIDropDownMenu_Initialize(mcf.Filter, function(...)
            self:InitializeDropDown(...)
        end);
    ...
    elseif mcf.selectedTab == ICATEGORIES then
        ...
        UIDropDownMenu_Initialize(mcf.Filter, function(...)
            self:InitializeDropDownForCats(...)
        end);
    else
        --[===[@debug@
...
function addon:OnItemDropped(itemButton)
    ...
    if currentTab == INEED then
        ...
        if index ~= NONAME then
            self:LoadItem(itemButton, itemLink, db.toons[index].requests, index)
        end
    elseif currentTab == ICATEGORIES then
        local index = currentCategory
        if index ~= NONAME then
            self:LoadItem(itemButton, itemLink, dbcategory[index].list, index)
        end
    else
        return
    end
...
function addon:Reset(input, ...)
    ...
    addon:Popup(message, 0, function(this)
...
do
    ...
    function addon:ShowAddCustomToon()
        if not dialogName then
            dialogName = self:BuildAddContact()
        end
...
do
    ...
    function addon:ShowAddCategory()
        if not dialogName then
            dialogName = self:BuildAddCategory()
        end
        StaticPopup_Show(dialogName, "", "")
    end
end
...
do
    ...
    function addon:ShowAddItemid()
        if not dialogName then
            dialogName = self:BuildAddItemid()
        end
        StaticPopup_Show(dialogName, "", "")
    end
end
